...
- Historical context (not product copy): anonymous provision + later human claim, similar to Claimable Neon
- Slug: `auth-md-agent-signup`

## Goal (single-user MVP)
In under two minutes, a reviewer can watch a **mock agent** discover Harbor Notes via `/auth.md`, register (anonymous or `service_auth`), request scopes, and wait for credentials — while a **human claim desk** approves the pending request with a 6-digit `user_code`. Status of pending/claimed agents and demo-safe fake tokens is visible the whole time. No production identity APIs.

## Out of scope
- Real WorkOS AuthKit or other production identity APIs
...
- [ ] No calls to production identity APIs
- [ ] Running product copy does not mention Neon / Claimable Neon
- [ ] Claim desk accepts a custom signed-in email and can approve a claim bound to that address
